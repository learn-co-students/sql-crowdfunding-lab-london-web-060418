...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS total_difference FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY(projects.title) HAVING(SUM(pledges.amount)) >= projects.funding_goal;"
end
